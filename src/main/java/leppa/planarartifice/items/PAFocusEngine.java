...
public class PAFocusEngine {
    ...
    public static void setCooldown(EntityLivingBase entityLiving, int cd) {
        if (cd == 0) {
            ...
            cooldownClient.put(entityLiving.getEntityId(), System.currentTimeMillis() + (cd * 50L));
        else cooldownServer.put(entityLiving.getEntityId(), System.currentTimeMillis() + (cd * 50L));
    }
    ...
    static boolean isOnCooldown(EntityLivingBase entityLiving) {
        ...
        if (!entityLiving.world.isRemote && cooldownServer.containsKey(entityLiving.getEntityId()))
